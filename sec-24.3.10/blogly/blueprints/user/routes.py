...
def list_users():
    ...
    return render_template("user/list.html.jinja", users=User.get_all(select))

@bp.get("/users/<int:id>")
def show_user(id):
    return render_template("user/show.html.jinja", user=User.get_or_404(id))
...
def show_edit_user_form(id):
    return render_template("user/edit.html.jinja", user=User.query.get_or_404(id))
